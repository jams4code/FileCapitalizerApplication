...
export interface IFileClient {
    import(formFile: FileParameter | null | undefined): Observable<FileResponse>;
    get(): Observable<ImportedFile[]>;
}
        ...
        return this.http.request("get", url_, options_).pipe(_observableMergeMap((response_ : any) => {
            ...
            if (response_ instanceof HttpResponseBase) {
                try {
                    ...
                    return <Observable<ImportedFile[]>><any>_observableThrow(e);
                }
            } else
                return <Observable<ImportedFile[]>><any>_observableThrow(response_);
        }));
    }

    protected processGet(response: HttpResponseBase): Observable<ImportedFile[]> {
        const status = response.status;
        const responseBlob = 
            response instanceof HttpResponse ? response.body : 
            (<any>response).error instanceof Blob ? (<any>response).error : undefined;
        ...
        if (status === 200) {
            ...
            if (Array.isArray(resultData200)) {
                ...
                for (let item of resultData200)
                    result200!.push(ImportedFile.fromJS(item));
            }
        ...
        return _observableOf<ImportedFile[]>(<any>null);
    }
}
...
export class ImportedFile extends AuditableEntity implements IImportedFile {
    id?: string;
    inputFileName?: string | undefined;
    fileSize?: number;

    constructor(data?: IImportedFile) {
        super(data);
    }
    ...
    init(_data?: any) {
        ...
        if (_data) {
            this.id = _data["id"];
            this.inputFileName = _data["inputFileName"];
            this.fileSize = _data["fileSize"];
        }
    }

    static fromJS(data: any): ImportedFile {
        data = typeof data === 'object' ? data : {};
        let result = new ImportedFile();
        result.init(data);
        return result;
    }
...
export interface IImportedFile extends IAuditableEntity {
    id?: string;
    inputFileName?: string | undefined;
    fileSize?: number;
}
